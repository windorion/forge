...
- Keep-by-default history retention and ordered SQLite migrations. Audit
  exports now carry content/source hashes and exact task-revision evidence.
  Only terminal tasks with explicit confirmation and a matching saved-export
  receipt can purge bounded command-output bodies; statuses, exit codes,
...
- Verified destructive-migration backup and offline restore. Every migration
  ...
  destructive SQL, SQLite `VACUUM INTO` must produce an owner-only snapshot
  whose integrity, schema, task count, bytes, and SHA-256 match a versioned
  manifest; failure starts no migration transaction. The offline CLI rejects
...
- Functional `4a` Mission Control routing slice. The handoff-aligned 1240px
  ...
  activity evidence. Its Commands tab now consumes the owning runtime's exact
  validation/command permission envelope for preset approval/run, known-command
  run/cancel, and reviewed self-fix rerun. Its Git tab loads repository-local
  status/diff and fresh commit, branch, publish, push, and PR preflights, then
  routes separately confirmed local commit/branch/non-force publish/push
  actions. Every background mutation performs a fresh identity and
  authorization check and shares a per-repository in-flight gate; stale Git
  evidence fails before request construction. Observer detail remains
  read-only. Authorized background children now use supervised
  ...
  command approval/run/cancel, cancelled-run persistence, scoped Git status/
  diff/commit/branch, cross-repository isolation, and remote-less publish/push/
...
- Centralized Secret Redaction policy v1 now protects retained/copyable
  ...
  chunks/summaries are redacted before SSE and SQLite, provider and HTTP
  failures plus Git/config diagnostics use the same classifier, audit exports
  name the exact policy version, persistence clones sanitize evidence fields
  without rewriting executable proposal bodies, and native copied diagnostics
  apply the matching policy. Direct, structured, private-key, credential-URL,
  percent/base64-encoded fixtures and explicit false-positive controls pass in
  pure TypeScript, isolated restart/SQLite E2E, and Swift tests.
...
- Command/preset approval lifecycle is fail-closed and auditable. New
  approvals are task-scoped bounded grants (one-hour default; fixed choices up
  to 24 hours), while repository and session scopes are described but not
  grantable by this endpoint. Expiry and append-only revocation appear in the
  runtime permission envelope and native contract/UI, are checked immediately
  before command start and between validation commands, and remain invalid
  across SQLite restart. Legacy approvals without scope/expiry are treated as
  expired. Revoking during an active process blocks future starts without
  turning revocation into implicit cancellation.
...
- macOS distribution posture policy v1 inventories every current app/runtime/
  login-item/CLI/Widget/Keychain/update component and exact entitlement state.
  Non-launching assembly cleans stale Runtime output and removes SwiftPM's
  debug ad-hoc signature; a no-overwrite clean stage supports full-bundle
  ad-hoc signing. Real artifact checks verify unsigned/ad-hoc profiles and must
  reject them as Developer ID releases. The placeholder feed now reports its
  missing signature honestly and cannot cross the install boundary. The policy
  still identifies external Node, production archive/export setup, Developer
  ID/hardened-runtime/notarization, signed updates/DMG, and WidgetKit as release
  gaps.
- Release-shaped unsigned signing-input foundation. A second versioned policy
  pins Node.js 22.18.0 official arm64/x86_64 archives, hashes, license, and
  executable paths without downloading them. The production path builds
  optimized app and standalone CLI Mach-O code, strips compiler signatures,
  rebuilds only distributable Runtime JavaScript, and rejects debug/test/
  source/database artifacts. It emits a per-file component inventory, SPDX 2.3
  SBOM, Runtime supply-chain copy, SHA256SUMS, normalized timestamps/owners, and
  byte-identical ustar+gzip archives. Real local arm64 evidence contains 106
  files and independently confirms both code boundaries are unsigned; hosted
  CI rebuilds, inspects, re-archives, structurally ad-hoc-signs app/CLI, and
  retains the Developer ID negative control. Pinned Node remains deliberately
  unbundled until its nested-code signing review is explicit.
...
| Commercial beta | 38-45% | Versioned migration/backup/restore, retention/export/purge, bounded/revocable approvals, centralized Secret Redaction, performance/security gates, signing/update threat checks, and an optimized deterministic unsigned signing-input pipeline with pinned Runtime supply-chain/SBOM evidence now exist; Forge still needs bundled Node, an actual signed/notarized package, production onboarding/GitHub/provider proof, packaged-app profiling, operations polish, and repeated real-repository success. |
| Polished v1 product | 35-42% | The real queue, local indexes, explicit data lifecycle, session-authorized runtimes, background task/command/Git review routing, fair supervised grants, bounded reconnect/crash recovery telemetry, reportable soak tooling, centralized Secret Redaction, compiled XCUITest harness, and release-shaped distribution foundation exist; full-duration/passing evidence, signed distribution, semantic memory, hosted collaboration, WidgetKit, and commercial polish remain. |
...
| Security, permissions, and audit | 99% | 1% | Explicit plan/edit/command/git/PR gates, bounded task grants, observer runtimes, owner-only writer/backup artifacts, verified restore, centralized Secret Redaction, deterministic versioned task/workspace export receipts, terminal-only history purge, unfinished-evidence preservation, bounded PR refresh, and versioned fail-closed signing/entitlement/update checks. | Repository/session approval stores only if product evidence needs them, commercial privacy text, and live signed-package proof. |
...
| Native macOS behavior and integrations | 87-92% | 8-13% | SwiftUI app, menus, shortcuts, notifications, Spotlight, CLI, onboarding, settings, managed runtimes, fair-queue state, rendered routed-tab transitions, three compiled `XCUIApplication` methods, one passing action-level path, and optimized non-launching app/CLI release assembly with deterministic signing-input archives. | Archive the full UI-test suite in an unattended window, final human-input checks, deeper Finder/IDE handoff, bundled Runtime, WidgetKit, and live signed-package proof. |
...
| Edit, command, validation, and repair | 88-93% | 7-12% | Reviewed multi-file create/modify/delete, Unified Diff, rollback, bounded/revocable approved commands, repair/rerun, and repository-scoped background command control. | Broader source transformations, command catalogs, evidence-led repository/session approval stores if needed, and post-rollback validation. |
...
| Distribution, updates, and operations | 45-55% | 45-55% | App-managed Runtime resource path, update UI, honest disabled feed, versioned signing profiles, exact Node.js 22.18.0 arm64/x86_64 supply-chain pins, optimized app/CLI release builds, debug/test exclusion, component/SPDX/checksum evidence, byte-deterministic unsigned archives, real unsigned/ad-hoc checks, and hosted release-negative control. | Safely ingest/re-sign bundled Node, production archive/export signing, Developer ID/hardened runtime, notarization/stapling, DMG, signed appcast install/relaunch, crash reporting, and clean-machine rehearsal. |
...
- broader project-command catalogs and evidence-led repository/session grant
  stores only if task-scoped lifecycle data proves they are needed
