...
| Security lifecycle | Beta | Explicit approvals, read-only observers, scoped authorization, Keychain secrets, bounded task-command grants, append-only revocation, restart proof, versioned Secret Redaction, and versioned workspace retention/export/purge with unfinished-evidence preservation are enforced by hosted security checks. | Signed-build threat review; repository/session validation grants remain deliberately unimplemented until evidence requires them. | Keep approval/redaction/retention regressions green and complete a machine-checkable app/runtime/helper/widget signing and entitlement review. |
...
retention/export/purge slices are now implemented; signed-build threat review
remains. It should not preempt the live-model Alpha
