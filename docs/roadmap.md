...
| Security lifecycle | Beta | Explicit approvals, read-only observers, scoped authorization, Keychain secrets, bounded task-command grants with one-hour default/fixed durations, append-only revocation, pre-spawn and between-command checks, restart proof, and versioned Secret Redaction across command/SSE/SQLite/audit/Provider-error/native-diagnostic evidence with encoded and false-positive fixtures. The hosted security workflow enforces both slices. | Workspace-wide retention and signed-build threat-review decisions; repository/session validation grants remain deliberately unimplemented until evidence requires them. | Keep approval and redaction regressions green, then complete workspace audit purge/retention policy and signed-build threat review. |
...
task-approval lifecycle, and broader Secret Redaction slices are now
implemented, but the workspace-wide policy/export/purge and signed-build
threat-review decisions remain. None should preempt the live-model Alpha
corpus, and each remaining exit condition must be resolved before claiming the
