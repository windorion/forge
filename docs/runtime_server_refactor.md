...
| --- | --- | --- |
| 0. Freeze observable behavior | Complete | The frozen refactor baseline had 55 routes; the current manifest has 58 after task cancellation/audit export and task detail, with HTTP/SSE, build, unit, coverage, and 19 smoke scripts passing |
...
- compatibility suite: 19 smoke scripts, all passing
...
- `runtimeRoutes.ts` is a 107-line route composer over system, task, agent,
  ...
  test scans those groups and currently proves all 58 routes.
...
unit coverage, all 19 runtime smoke scripts, and 36 Swift tests. The current
